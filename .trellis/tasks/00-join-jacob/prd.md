...
`Jacob` just ran `trellis init` on a fresh clone, saw "Developer
...
- `.trellis/workspace/Jacob/journal-*.md` — their session log,
...
- Check if `.trellis/workspace/Jacob/` already exists — if yes, it's
  their journal from another machine and worth mentioning.
- Run `python3 ./.trellis/scripts/task.py list --assignee Jacob` to
...
python3 ./.trellis/scripts/task.py archive 00-join-jacob
```
